...
table_name = 'task_trays'

src_task_trays  = f"SELECT * FROM task_trays WHERE patient_id IN ({','.join(map(str, patient_ids))})"
src_task_trays_df = pd.read_sql(src_task_trays, src_connection)
...
#---------------------------patient_id----------------------------------
# Merging source task_trays with target patients mapping to get target patient_id
src_task_trays_df['patient_id'] = src_task_trays_df['patient_id'].astype(int)
tgt_mapping_patient_table_df['source_id'] = tgt_mapping_patient_table_df['source_id'].astype(int)
src_task_trays_df = pd.merge(src_task_trays_df, tgt_mapping_patient_table_df, left_on='patient_id', right_on='source_id', how='left')
src_task_trays_df = src_task_trays_df.drop(columns=['patient_id', 'source_id'])
src_task_trays_df = src_task_trays_df.rename(columns={'target_id': 'patient_id'})

#----------------------------new data insertion--------------------------------
src_task_trays_df0 = src_task_trays_df.drop_duplicates()
src_task_trays_df1 = src_task_trays_df0
#id genration for new data
tgt_task_trays_max = f'SELECT CASE WHEN MAX(id) is NULL THEN 1 ELSE MAX(id) + 1 END as max_id FROM {table_name}'
tgt_task_trays_max_df = pd.read_sql(tgt_task_trays_max, tgt_connection)
max_id = int(tgt_task_trays_max_df['max_id'].iloc[0])
src_task_trays_df1.insert(0, 'target_id', range(max_id, max_id + len(src_task_trays_df1)))
...
insert_new_records_and_mapping(src_task_trays_df1, tgt_connection, practice_name, table_name)
