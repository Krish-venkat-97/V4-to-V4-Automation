...
src_receipt_details  = f"SELECT id.* FROM receipt_details id INNER JOIN invoices i On id.invoice_id = i.id WHERE i.patient_id IN ({','.join(map(str, patient_ids))})"
src_receipt_details_df = pd.read_sql(src_receipt_details, src_connection)

tgt_mapping_invoice_table = f"SELECT source_id, target_id FROM mapping_table WHERE source = '{getPractice()}' AND table_name = 'invoices'"
tgt_mapping_invoice_table_df = pd.read_sql(tgt_mapping_invoice_table, tgt_connection)

tgt_mapping_receipt_table = f"SELECT source_id, target_id FROM mapping_table WHERE source = '{getPractice()}' AND table_name = 'receipts'"
tgt_mapping_receipt_table_df = pd.read_sql(tgt_mapping_receipt_table, tgt_connection)

#----------------------------invoice_id-----------------------------------
# Merging source receipt_details with target invoices mapping to get target invoice_id
if src_receipt_details_df['invoice_id'].isnull().all():
    src_receipt_details_df['invoice_id'] = None
else:
    src_receipt_details_df['invoice_id'] = src_receipt_details_df['invoice_id'].apply(lambda x: int(x) if pd.notnull(x) else None)
    tgt_mapping_invoice_table_df['source_id'] = tgt_mapping_invoice_table_df['source_id'].apply(lambda x: int(x) if pd.notnull(x) else None)
    src_receipt_details_df = pd.merge(src_receipt_details_df, tgt_mapping_invoice_table_df, left_on='invoice_id', right_on='source_id', how='left')
    src_receipt_details_df = src_receipt_details_df.drop(columns=['invoice_id', 'source_id'])
    src_receipt_details_df = src_receipt_details_df.rename(columns={'target_id': 'invoice_id'})

#----------------------------receipt_id-----------------------------------
# Merging source receipt_details with target receipts mapping to get target receipt_id
if src_receipt_details_df['receipt_id'].isnull().all():
    src_receipt_details_df['receipt_id'] = None
else:
    src_receipt_details_df['receipt_id'] = src_receipt_details_df['receipt_id'].apply(lambda x: int(x) if pd.notnull(x) else None)
    tgt_mapping_receipt_table_df['source_id'] = tgt_mapping_receipt_table_df['source_id'].apply(lambda x: int(x) if pd.notnull(x) else None)
    src_receipt_details_df = pd.merge(src_receipt_details_df, tgt_mapping_receipt_table_df, left_on='receipt_id', right_on='source_id', how='left')
    src_receipt_details_df = src_receipt_details_df.drop(columns=['receipt_id', 'source_id'])
    src_receipt_details_df = src_receipt_details_df.rename(columns={'target_id': 'receipt_id'})

#----------------------------new data insertion--------------------------------
src_receipt_details_df1 = src_receipt_details_df
#id genration for new data
tgt_receipt_details_max = f'SELECT CASE WHEN MAX(id) is NULL THEN 1 ELSE MAX(id) + 1 END as max_id FROM {table_name}'
tgt_receipt_details_max_df = pd.read_sql(tgt_receipt_details_max, tgt_connection)
max_id = int(tgt_receipt_details_max_df['max_id'].iloc[0])
src_receipt_details_df1.insert(0, 'target_id', range(max_id, max_id + len(src_receipt_details_df1)))
...
insert_new_records_and_mapping(src_receipt_details_df1, tgt_connection, practice_name, table_name)
