...
table_name = 'waiting_lists'

src_waiting_lists  = f"SELECT * FROM waiting_lists WHERE patient_id IN ({','.join(map(str, patient_ids))})"
src_waiting_lists_df = pd.read_sql(src_waiting_lists, src_connection)
...
tgt_mapping_appointment_description_table = f"SELECT source_id, target_id FROM mapping_table WHERE source = '{getPractice()}' AND table_name = 'appointment_descriptions'"
tgt_mapping_appointment_description_table_df = pd.read_sql(tgt_mapping_appointment_description_table, tgt_connection)
...
#---------------------------patient_id----------------------------------
# Merging source waiting_lists with target patients mapping to get target patient_id
src_waiting_lists_df['patient_id'] = src_waiting_lists_df['patient_id'].astype(int)
tgt_mapping_patient_table_df['source_id'] = tgt_mapping_patient_table_df['source_id'].astype(int)
src_waiting_lists_df = pd.merge(src_waiting_lists_df, tgt_mapping_patient_table_df, left_on='patient_id', right_on='source_id', how='left')
src_waiting_lists_df = src_waiting_lists_df.drop(columns=['patient_id', 'source_id'])
src_waiting_lists_df = src_waiting_lists_df.rename(columns={'target_id': 'patient_id'})

#----------------------------Appointment description_id---------------------------
# Merging source waiting_lists description with target appointment descriptions mapping to get target appointment_description_id
src_waiting_lists_df['appointment_description_id'] = src_waiting_lists_df['appointment_description_id'].apply(lambda x: int(x) if pd.notnull(x) else None)
tgt_mapping_appointment_description_table_df['source_id'] = tgt_mapping_appointment_description_table_df['source_id'].astype(int)
src_waiting_lists_df = pd.merge(src_waiting_lists_df, tgt_mapping_appointment_description_table_df, left_on='appointment_description_id', right_on='source_id', how='left')
src_waiting_lists_df = src_waiting_lists_df.drop(columns=['appointment_description_id', 'source_id'])
src_waiting_lists_df = src_waiting_lists_df.rename(columns={'target_id': 'appointment_description_id'})

#----------------------------procedure_id-----------------------------------
# Merging source waiting_lists with target procedures mapping to get target procedure_id
if src_waiting_lists_df['procedure_id'].isnull().all():
    src_waiting_lists_df['procedure_id'] = None
else:
    src_waiting_lists_df['procedure_id'] = src_waiting_lists_df['procedure_id'].apply(lambda x: int(x) if pd.notnull(x) else None)
    tgt_mapping_procedure_table_df['source_id'] = tgt_mapping_procedure_table_df['source_id'].apply(lambda x: int(x) if pd.notnull(x) else None)
    src_waiting_lists_df = pd.merge(src_waiting_lists_df, tgt_mapping_procedure_table_df, left_on='procedure_id', right_on='source_id', how='left')
    src_waiting_lists_df = src_waiting_lists_df.drop(columns=['procedure_id', 'source_id'])
    src_waiting_lists_df = src_waiting_lists_df.rename(columns={'target_id': 'procedure_id'})

#----------------------------hospital_id-----------------------------------
# Merging source waiting_lists with target hospitals mapping to get target hospital_id
if src_waiting_lists_df['location_id'].isnull().all():
    src_waiting_lists_df['location_id'] = None
else:
    src_waiting_lists_df['location_id'] = src_waiting_lists_df['location_id'].apply(lambda x: int(x) if pd.notnull(x) else None)
    ...
    src_waiting_lists_df = pd.merge(src_waiting_lists_df, tgt_mapping_hospital_table_df, left_on='location_id', right_on='source_id', how='left')
    src_waiting_lists_df = src_waiting_lists_df.drop(columns=['location_id', 'source_id'])
    src_waiting_lists_df = src_waiting_lists_df.rename(columns={'target_id': 'location_id'})

#----------------------------wrtting default values--------------------
#src_waiting_lists_df['invoice_id'] = 0

#----------------------------new data insertion--------------------------------
src_waiting_lists_df0 = src_waiting_lists_df.drop_duplicates()
src_waiting_lists_df1 = src_waiting_lists_df0
#id genration for new data
tgt_waiting_lists_max = f'SELECT CASE WHEN MAX(id) is NULL THEN 1 ELSE MAX(id) + 1 END as max_id FROM {table_name}'
tgt_waiting_lists_max_df = pd.read_sql(tgt_waiting_lists_max, tgt_connection)
max_id = int(tgt_waiting_lists_max_df['max_id'].iloc[0])
src_waiting_lists_df1.insert(0, 'target_id', range(max_id, max_id + len(src_waiting_lists_df1)))
...
insert_new_records_and_mapping(src_waiting_lists_df1, tgt_connection, practice_name, table_name)
