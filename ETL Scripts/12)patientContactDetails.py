...
tgt_patient_contact_details_max = f'SELECT CASE WHEN MAX(id) is NULL THEN 1 ELSE MAX(id) + 1 END as max_id FROM {table_name}'
tgt_patient_contact_details_max_df = pd.read_sql(tgt_patient_contact_details_max, tgt_connection)
max_id = int(tgt_patient_contact_details_max_df['max_id'].iloc[0])
