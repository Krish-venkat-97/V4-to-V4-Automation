...
src_forms  = f"SELECT * FROM forms WHERE patient_id IN ({','.join(map(str, patient_ids))})"
src_forms_df = pd.read_sql(src_forms, src_connection)
...
#---------------------------patient_id----------------------------------
# Merging source forms with target patients mapping to get target patient_id
src_forms_df['patient_id'] = src_forms_df['patient_id'].astype(int)
tgt_mapping_patient_table_df['source_id'] = tgt_mapping_patient_table_df['source_id'].astype(int)
src_forms_df = pd.merge(src_forms_df, tgt_mapping_patient_table_df, left_on='patient_id', right_on='source_id', how='left')
src_forms_df = src_forms_df.drop(columns=['patient_id', 'source_id'])
src_forms_df = src_forms_df.rename(columns={'target_id': 'patient_id'})

#----------------------------episode_id-----------------------------------
# Merging source forms with target episodes mapping to get target episode_id
if src_forms_df['episode_id'].isnull().all():
    src_forms_df['episode_id'] = None
else:
    src_forms_df['episode_id'] = src_forms_df['episode_id'].apply(lambda x: int(x) if pd.notnull(x) else None)
    ...
    src_forms_df = pd.merge(src_forms_df, tgt_mapping_episode_table_df, left_on='episode_id', right_on='source_id', how='left')
    src_forms_df = src_forms_df.drop(columns=['episode_id', 'source_id'])
    src_forms_df = src_forms_df.rename(columns={'target_id': 'episode_id'})

#----------------------------new data insertion--------------------------------
src_forms_df1 = src_forms_df
#id generation for new data
tgt_forms_max = f'SELECT CASE WHEN MAX(id) is NULL THEN 1 ELSE MAX(id) + 1 END as max_id FROM {table_name}'
tgt_forms_max_df = pd.read_sql(tgt_forms_max, tgt_connection)
max_id = int(tgt_forms_max_df['max_id'].iloc[0])
src_forms_df1.insert(0, 'target_id', range(max_id, max_id + len(src_forms_df1)))
...
def insert_new_records_and_mapping(df, tgt_connection, practice_name, table_name):
    ...
    try:
        ...
        print('Data and mapping table insert successful for forms - New insert!')
    except Exception as e:
        logging.error(f"Data and mapping table insert failed for forms - New insert! Error occurred: {e} \n Query: {insert_query} \n Mapping Query: {insert_query1} \n sample row: {rows[0] if rows else 'No rows to insert'} \n sample mapping row: {mapping_rows[0] if mapping_rows else 'No mapping rows to insert'}", flush=True)
        print("Query:", insert_query)
        print("Sample Row:", rows[0] if rows else "No rows to insert")
        print('Data and mapping table insert failed for forms - New insert!')

# Insert new records and mapping entries simultaneously
insert_new_records_and_mapping(src_forms_df1, tgt_connection, practice_name, table_name)
