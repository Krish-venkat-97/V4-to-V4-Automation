...
src_contact_types  = f"SELECT * FROM contact_types"
src_contact_types_df = pd.read_sql(src_contact_types, src_connection)
#drop the duplicate names (case-insensitive)
src_contact_types_df['name_upper'] = src_contact_types_df['name'].str.upper()
src_contact_types_df = src_contact_types_df.drop_duplicates(subset=['name_upper'], keep='first')

tgt_contact_types = 'SELECT id as target_id,UPPER(name) as contact_type_name FROM contact_types'
tgt_contact_types_df = pd.read_sql(tgt_contact_types, tgt_connection)

#----------------------------new data insertion--------------------------------
src_contact_types_df1 = src_contact_types_df[~src_contact_types_df['name'].str.upper().isin(tgt_contact_types_df['contact_type_name'])]
#id genration for new data
tgt_contact_types_max = f'SELECT CASE WHEN MAX(id) is NULL THEN 1 ELSE MAX(id) + 1 END as max_id FROM {table_name}'
tgt_contact_types_max_df = pd.read_sql(tgt_contact_types_max, tgt_connection)
max_id = int(tgt_contact_types_max_df['max_id'].iloc[0])
src_contact_types_df1.insert(0, 'target_id', range(max_id, max_id + len(src_contact_types_df1)))
src_contact_types_df1 = src_contact_types_df1.drop(columns=['name_upper'])

#-------------------------------existing data mapping table update--------------------------------
# Merge the source and target dataframes to find existing records
src_contact_types_df['name_upper'] = src_contact_types_df['name'].str.upper()
src_contact_types_df2 = pd.merge(src_contact_types_df, tgt_contact_types_df, left_on='name_upper', right_on='contact_type_name', how='inner')
src_contact_types_df2 = src_contact_types_df2.drop(columns=['name_upper', 'contact_type_name'])
src_contact_types_df2 = src_contact_types_df2[['target_id','id']].rename(columns={'id': 'source_id'})
...
def insert_new_records_and_mapping(df, tgt_connection, practice_name, table_name):
    ...
    try:
        ...
        print('Data and mapping table insert successful for contact_types - New insert!')
    except Exception as e:
        logging.error(f"Data and mapping table insert failed for contact_types - New insert! Error occurred: {e} \n Query: {insert_query} \n Mapping Query: {insert_query1} \n sample row: {rows[0] if rows else 'No rows to insert'} \n sample mapping row: {mapping_rows[0] if mapping_rows else 'No mapping rows to insert'}")
        #print("Query:", insert_query)
        #print("Sample Row:", rows[0] if rows else "No rows to insert")
        print('Data and mapping table insert failed for contact_types - New insert!')

# Insert new records and mapping entries simultaneously
insert_new_records_and_mapping(src_contact_types_df1, tgt_connection, practice_name, table_name)
...
def update_existing_records_and_mapping(df, tgt_connection, practice_name, table_name):
    ...
    try:
        with tgt_connection.cursor() as cursor:
            ...
            cursor.executemany(insert_query1, mapping_rows)
            tgt_connection.commit()
        print('Data and mapping table insert successful for contact_types - Existing update!')
    except Exception as e:
        logging.error(f"Data and mapping table insert failed for contact_types - Existing update! Error occurred: {e} \n Mapping Query: {insert_query1} \n sample row: {rows[0] if rows else 'No rows to insert'}")
        #print("Query:", insert_query1)
        #print("Sample Row:", rows[0] if rows else "No rows to insert")
        print('Data and mapping table insert failed for contact_types - Existing update!')

# Update existing records and mapping entries simultaneously
update_existing_records_and_mapping(src_contact_types_df2, tgt_connection, practice_name, table_name)
