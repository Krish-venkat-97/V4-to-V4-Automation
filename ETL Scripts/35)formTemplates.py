...
src_form_templates  = f"SELECT * FROM form_templates"
src_form_templates_df = pd.read_sql(src_form_templates, src_connection)
#drop the duplicate names (case-insensitive)
src_form_templates_df['name_upper'] = src_form_templates_df['name'].str.upper()

tgt_form_templates = 'SELECT id as target_id,UPPER(name) as template_name FROM form_templates'
tgt_form_templates_df = pd.read_sql(tgt_form_templates, tgt_connection)

#----------------------------new data insertion--------------------------------
src_form_templates_df1 = src_form_templates_df[~src_form_templates_df['name'].str.upper().isin(tgt_form_templates_df['template_name'])]
#id genration for new data
tgt_form_templates_max = f'SELECT CASE WHEN MAX(id) is NULL THEN 1 ELSE MAX(id) + 1 END as max_id FROM {table_name}'
tgt_form_templates_max_df = pd.read_sql(tgt_form_templates_max, tgt_connection)
max_id = int(tgt_form_templates_max_df['max_id'].iloc[0])
src_form_templates_df1.insert(0, 'target_id', range(max_id, max_id + len(src_form_templates_df1)))

#-------------------------------existing data mapping table update--------------------------------
# Merge the source and target dataframes to find existing records
src_form_templates_df['name_upper'] = src_form_templates_df['name'].str.upper()
src_form_templates_df2 = pd.merge(src_form_templates_df, tgt_form_templates_df, left_on='name_upper', right_on='template_name', how='inner')
src_form_templates_df2 = src_form_templates_df2[['target_id','id']].rename(columns={'id': 'source_id'})
...
def insert_new_records_and_mapping(df, tgt_connection, practice_name, table_name):
    ...
    column_names = ['`key`' if col == 'key' else col for col in new_records_df.columns]
    ...
    try:
        ...
        print('Data and mapping table insert successful for form_templates - New insert!',flush=True)
    except Exception as e:
        logging.error(f"Data and mapping table insert failed for form_templates - New insert!: Error occurred: {e} \n Query: {insert_query} \n Mapping Query: {insert_query1} \n sample row: {rows[0] if rows else 'No rows to insert'}", flush=True)
        #print("Query:", insert_query)
        #print("Sample Row:", rows[0] if rows else "No rows to insert")
        print('Data and mapping table insert failed for form_templates - New insert!',flush=True)

# Insert new records and mapping entries simultaneously
insert_new_records_and_mapping(src_form_templates_df1, tgt_connection, practice_name, table_name)
...
def update_existing_records_and_mapping(df, tgt_connection, practice_name, table_name):
    ...
    try:
        with tgt_connection.cursor() as cursor:
            ...
            cursor.executemany(insert_query1, mapping_rows)
            tgt_connection.commit()
        print('Data and mapping table insert successful for form_templates - Existing update!')
    except Exception as e:
        logging.error(f"Data and mapping table insert failed for form_templates - Existing update! Error occurred: {e} \n Mapping Query: {insert_query1} \n sample row: {rows[0] if rows else 'No rows to insert'}", flush=True)
        #print("Query:", insert_query1)
        #print("Sample Row:", rows[0] if rows else "No rows to insert")
        print('Data and mapping table insert failed for form_templates - Existing update!',flush=True)

# Update existing records and mapping entries simultaneously
update_existing_records_and_mapping(src_form_templates_df2, tgt_connection, practice_name, table_name)
