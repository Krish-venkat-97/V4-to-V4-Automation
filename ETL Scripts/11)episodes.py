...
src_episodes  = f"SELECT * FROM episodes WHERE patient_id IN ({','.join(map(str, patient_ids))})"
src_episodes_df = pd.read_sql(src_episodes, src_connection)
...
#---------------------------patient_id----------------------------------
# Merging source episodes with target patients mapping to get target patient_id
src_episodes_df['patient_id'] = src_episodes_df['patient_id'].astype(int)
tgt_mapping_patient_table_df['source_id'] = tgt_mapping_patient_table_df['source_id'].astype(int)
src_episodes_df = pd.merge(src_episodes_df, tgt_mapping_patient_table_df, left_on='patient_id', right_on='source_id', how='left')
src_episodes_df = src_episodes_df.drop(columns=['patient_id', 'source_id'])
src_episodes_df = src_episodes_df.rename(columns={'target_id': 'patient_id'})

#----------------------------new data insertion--------------------------------
src_episodes_df1 = src_episodes_df
#id genration for new data
tgt_episodes_max = f'SELECT CASE WHEN MAX(id) is NULL THEN 1 ELSE MAX(id) + 1 END as max_id FROM {table_name}'
tgt_episodes_max_df = pd.read_sql(tgt_episodes_max, tgt_connection)
max_id = int(tgt_episodes_max_df['max_id'].iloc[0])
src_episodes_df1.insert(0, 'target_id', range(max_id, max_id + len(src_episodes_df1)))
...
def insert_new_records_and_mapping(df, tgt_connection, practice_name, table_name):
    ...
    try:
        ...
        print('Data and mapping table insert successful for episodes - New insert!')
    except Exception as e:
        logging.error(f"Data and mapping table insert failed for episodes - New insert! Error occurred: {e} \n Query: {insert_query} \n Mapping Query: {insert_query1} \n sample row: {rows[0] if rows else 'No rows to insert'} \n sample mapping row: {mapping_rows[0] if mapping_rows else 'No mapping rows to insert'}")
        #print("Query:", insert_query)
        #print("Sample Row:", rows[0] if rows else "No rows to insert")
        print('Data and mapping table insert failed for episodes - New insert!')

# Insert new records and mapping entries simultaneously
insert_new_records_and_mapping(src_episodes_df1, tgt_connection, practice_name, table_name)
