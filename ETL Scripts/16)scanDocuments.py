...
tgt_scan_documents_max = f'SELECT CASE WHEN MAX(id) is NULL THEN 1 ELSE MAX(id) + 1 END as max_id FROM {table_name}'
tgt_scan_documents_max_df = pd.read_sql(tgt_scan_documents_max, tgt_connection)
max_id = int(tgt_scan_documents_max_df['max_id'].iloc[0])
