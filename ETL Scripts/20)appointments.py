...
src_appointments  = f"SELECT * FROM appointments WHERE patient_id IN ({','.join(map(str, patient_ids))})"
...
#---------------------------patient_id----------------------------------
# Merging source appointments with target patients mapping to get target patient_id
src_appointments_df['patient_id'] = src_appointments_df['patient_id'].astype(int)
tgt_mapping_patient_table_df['source_id'] = tgt_mapping_patient_table_df['source_id'].astype(int)
src_appointments_df = pd.merge(src_appointments_df, tgt_mapping_patient_table_df, left_on='patient_id', right_on='source_id', how='left')
src_appointments_df = src_appointments_df.drop(columns=['patient_id', 'source_id'])
src_appointments_df = src_appointments_df.rename(columns={'target_id': 'patient_id'})

#----------------------------Appointment description_id---------------------------
# Merging source appointments description with target appointment descriptions mapping to get target appointment_description_id
src_appointments_df['appointment_description_id'] = src_appointments_df['appointment_description_id'].apply(lambda x: int(x) if pd.notnull(x) else None)
tgt_mapping_appointment_description_table_df['source_id'] = tgt_mapping_appointment_description_table_df['source_id'].astype(int)
src_appointments_df = pd.merge(src_appointments_df, tgt_mapping_appointment_description_table_df, left_on='appointment_description_id', right_on='source_id', how='left')
src_appointments_df = src_appointments_df.drop(columns=['appointment_description_id', 'source_id'])
src_appointments_df = src_appointments_df.rename(columns={'target_id': 'appointment_description_id'})

#----------------------------episode_id-----------------------------------
# Merging source appointments with target episodes mapping to get target episode_id
if src_appointments_df['episode_id'].isnull().all():
    src_appointments_df['episode_id'] = None
else:
    src_appointments_df['episode_id'] = src_appointments_df['episode_id'].apply(lambda x: int(x) if pd.notnull(x) else None)
    ...
    src_appointments_df = pd.merge(src_appointments_df, tgt_mapping_episode_table_df, left_on='episode_id', right_on='source_id', how='left')
    src_appointments_df = src_appointments_df.drop(columns=['episode_id', 'source_id'])
    src_appointments_df = src_appointments_df.rename(columns={'target_id': 'episode_id'})

#----------------------------wrtting default values--------------------
src_appointments_df['invoice_id'] = 0

#----------------------------new data insertion--------------------------------
src_appointments_df1 = src_appointments_df
#id genration for new data
tgt_appointments_max = f'SELECT CASE WHEN MAX(id) is NULL THEN 1 ELSE MAX(id) + 1 END as max_id FROM {table_name}'
tgt_appointments_max_df = pd.read_sql(tgt_appointments_max, tgt_connection)
max_id = int(tgt_appointments_max_df['max_id'].iloc[0])
src_appointments_df1.insert(0, 'target_id', range(max_id, max_id + len(src_appointments_df1)))
