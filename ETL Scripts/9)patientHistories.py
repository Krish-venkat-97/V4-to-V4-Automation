...
table_name = 'personal_histories'

src_personal_histories  = f"SELECT * FROM personal_histories WHERE patient_id IN ({','.join(map(str, patient_ids))})"
src_personal_histories_df = pd.read_sql(src_personal_histories, src_connection)
...
#---------------------------patient_id----------------------------------
# Merging source personal_histories with target patients mapping to get target patient_id
src_personal_histories_df['patient_id'] = src_personal_histories_df['patient_id'].astype(int)
tgt_mapping_patient_table_df['source_id'] = tgt_mapping_patient_table_df['source_id'].astype(int)
src_personal_histories_df = pd.merge(src_personal_histories_df, tgt_mapping_patient_table_df, left_on='patient_id', right_on='source_id', how='left')
src_personal_histories_df = src_personal_histories_df.drop(columns=['patient_id', 'source_id'])
src_personal_histories_df = src_personal_histories_df.rename(columns={'target_id': 'patient_id'})

#----------------------------new data insertion--------------------------------
src_personal_histories_df1 = src_personal_histories_df
#id genration for new data
tgt_personal_histories_max = f'SELECT CASE WHEN MAX(id) is NULL THEN 1 ELSE MAX(id) + 1 END as max_id FROM {table_name}'
tgt_personal_histories_max_df = pd.read_sql(tgt_personal_histories_max, tgt_connection)
max_id = int(tgt_personal_histories_max_df['max_id'].iloc[0])
src_personal_histories_df1.insert(0, 'target_id', range(max_id, max_id + len(src_personal_histories_df1)))
...
def insert_new_records_and_mapping(df, tgt_connection, practice_name, table_name):
    ...
    try:
        ...
        print('Data and mapping table insert successful for personal_histories - New insert!')
    except Exception as e:
        logging.error(f" Data and mapping table insert failed for personal_histories - New insert Error occurred: {e} \n Query: {insert_query} \n Mapping Query: {insert_query1} \n sample row: {rows[0] if rows else 'No rows to insert'} \n sample mapping row: {mapping_rows[0] if mapping_rows else 'No mapping rows to insert'}", flush=True)
        #print("Query:", insert_query)
        #print("Sample Row:", rows[0] if rows else "No rows to insert")
        #print('Data and mapping table insert failed for personal_histories - New insert!')

# Insert new records and mapping entries simultaneously
insert_new_records_and_mapping(src_personal_histories_df1, tgt_connection, practice_name, table_name)
