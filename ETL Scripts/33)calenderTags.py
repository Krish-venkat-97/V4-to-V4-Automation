...
table_name = 'calendar_tags'

src_tags  = f"SELECT * FROM calendar_tags"
src_tags_df = pd.read_sql(src_tags, src_connection)
#drop the duplicate names (case-insensitive)
#src_tags_df['name_upper'] = src_tags_df['name'].str.upper()
#src_tags_df = src_tags_df.drop_duplicates(subset=['name_upper'], keep='first')

tgt_tags = 'SELECT id as target_id,UPPER(name) as tag_name,doctor_id,from_date,to_date FROM calendar_tags'
tgt_tags_df = pd.read_sql(tgt_tags, tgt_connection)

#----------------------------new data insertion--------------------------------
#src_tags_df1 = src_tags_df[~src_tags_df['name'].str.upper().isin(tgt_tags_df['tag_name'])]

src_tags_df['name_upper'] = src_tags_df['name'].str.upper()
compare_cols = ['name_upper', 'doctor_id', 'from_date', 'to_date']

src_tags_df1 = src_tags_df.merge(
    tgt_tags_df,
    how='left',
    left_on=compare_cols,
    right_on=['tag_name', 'doctor_id', 'from_date', 'to_date'],
    indicator=True
).query('_merge == "left_only"').drop(columns=['_merge', 'tag_name','name_upper','target_id'])

#id genration for new data
tgt_tags_max = f'SELECT CASE WHEN MAX(id) is NULL THEN 1 ELSE MAX(id) + 1 END as max_id FROM {table_name}'
tgt_tags_max_df = pd.read_sql(tgt_tags_max, tgt_connection)
max_id = int(tgt_tags_max_df['max_id'].iloc[0])
src_tags_df1.insert(0, 'target_id', range(max_id, max_id + len(src_tags_df1)))

#-------------------------------existing data mapping table update--------------------------------
# Merge the source and target dataframes to find existing records
#src_tags_df['name_upper'] = src_tags_df['name'].str.upper()
#src_tags_df2 = pd.merge(src_tags_df, tgt_tags_df, left_on='name_upper', right_on='tag_name', how='inner')

src_tags_df2 = src_tags_df.merge(
    tgt_tags_df,
    how='inner',
    left_on=compare_cols,
    right_on=['tag_name', 'doctor_id', 'from_date', 'to_date'],
    indicator=True
).drop(columns=['_merge', 'tag_name','name_upper'])

src_tags_df2 = src_tags_df2[['target_id','id']].rename(columns={'id': 'source_id'})
...
def insert_new_records_and_mapping(df, tgt_connection, practice_name, table_name):
    ...
    try:
        ...
        print('Data and mapping table insert successful for tags - New insert!',flush=True)
    except Exception as e:
        logging.error(f"Data and mapping table insert failed for tags - New insert!: Error occurred: {e} \n Query: {insert_query} \n Mapping Query: {insert_query1} \n sample row: {rows[0] if rows else 'No rows to insert'}", flush=True)
        #print("Query:", insert_query)
        #print("Sample Row:", rows[0] if rows else "No rows to insert")
        print('Data and mapping table insert failed for tags - New insert!',flush=True)

# Insert new records and mapping entries simultaneously
insert_new_records_and_mapping(src_tags_df1, tgt_connection, practice_name, table_name)
...
def update_existing_records_and_mapping(df, tgt_connection, practice_name, table_name):
    ...
    try:
        with tgt_connection.cursor() as cursor:
            ...
            cursor.executemany(insert_query1, mapping_rows)
            tgt_connection.commit()
        print('Data and mapping table insert successful for tags - Existing update!')
    except Exception as e:
        logging.error(f"Data and mapping table insert failed for tags - Existing update! Error occurred: {e} \n Mapping Query: {insert_query1} \n sample row: {rows[0] if rows else 'No rows to insert'}", flush=True)
        #print("Query:", insert_query1)
        #print("Sample Row:", rows[0] if rows else "No rows to insert")
        print('Data and mapping table insert failed for tags - Existing update!',flush=True)

# Update existing records and mapping entries simultaneously
update_existing_records_and_mapping(src_tags_df2, tgt_connection, practice_name, table_name)
