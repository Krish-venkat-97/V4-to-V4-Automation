...
src_invoice_details  = f"SELECT id.* FROM invoice_details id INNER JOIN invoices i On id.invoice_id = i.id WHERE i.patient_id IN ({','.join(map(str, patient_ids))})"
src_invoice_details_df = pd.read_sql(src_invoice_details, src_connection)
...
#----------------------------procedure_id-----------------------------------
# Merging source invoice_details with target prcoedures mapping to get target procedure_id
if src_invoice_details_df['procedure_id'].isnull().all():
    src_invoice_details_df['procedure_id'] = None
else:
    src_invoice_details_df['procedure_id'] = src_invoice_details_df['procedure_id'].apply(lambda x: int(x) if pd.notnull(x) else None)
    ...
    src_invoice_details_df = pd.merge(src_invoice_details_df, tgt_mapping_procedure_table_df, left_on='procedure_id', right_on='source_id', how='left')
    src_invoice_details_df = src_invoice_details_df.drop(columns=['procedure_id', 'source_id'])
    src_invoice_details_df = src_invoice_details_df.rename(columns={'target_id': 'procedure_id'})

#----------------------------invoice_id-----------------------------------
# Merging source invoice_details with target invoices mapping to get target invoice_id
if src_invoice_details_df['invoice_id'].isnull().all():
    src_invoice_details_df['invoice_id'] = None
else:
    src_invoice_details_df['invoice_id'] = src_invoice_details_df['invoice_id'].apply(lambda x: int(x) if pd.notnull(x) else None)
    ...
    src_invoice_details_df = pd.merge(src_invoice_details_df, tgt_mapping_invoice_table_df, left_on='invoice_id', right_on='source_id', how='left')
    src_invoice_details_df = src_invoice_details_df.drop(columns=['invoice_id', 'source_id'])
    src_invoice_details_df = src_invoice_details_df.rename(columns={'target_id': 'invoice_id'})

#----------------------------new data insertion--------------------------------
src_invoice_details_df1 = src_invoice_details_df
#id genration for new data
tgt_invoice_details_max = f'SELECT CASE WHEN MAX(id) is NULL THEN 1 ELSE MAX(id) + 1 END as max_id FROM {table_name}'
tgt_invoice_details_max_df = pd.read_sql(tgt_invoice_details_max, tgt_connection)
max_id = int(tgt_invoice_details_max_df['max_id'].iloc[0])
src_invoice_details_df1.insert(0, 'target_id', range(max_id, max_id + len(src_invoice_details_df1)))
...
insert_new_records_and_mapping(src_invoice_details_df1, tgt_connection, practice_name, table_name)
