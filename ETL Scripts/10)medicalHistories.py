...
src_medical_histories  = f"SELECT * FROM medical_histories WHERE patient_id IN ({','.join(map(str, patient_ids))})"
src_medical_histories_df = pd.read_sql(src_medical_histories, src_connection)
...
#---------------------------patient_id----------------------------------
# Merging source medical_histories with target patients mapping to get target patient_id
src_medical_histories_df['patient_id'] = src_medical_histories_df['patient_id'].astype(int)
tgt_mapping_patient_table_df['source_id'] = tgt_mapping_patient_table_df['source_id'].astype(int)
src_medical_histories_df = pd.merge(src_medical_histories_df, tgt_mapping_patient_table_df, left_on='patient_id', right_on='source_id', how='left')
src_medical_histories_df = src_medical_histories_df.drop(columns=['patient_id', 'source_id'])
src_medical_histories_df = src_medical_histories_df.rename(columns={'target_id': 'patient_id'})
...
def insert_new_records_and_mapping(df, tgt_connection, practice_name, table_name):
    ...
    try:
        ...
        print('Data and mapping table insert successful for medical_histories - New insert!')
    except Exception as e:
        logging.error(f"Data and mapping table insert failed for medical_histories - New insert! Error occurred: {e} \n query: {insert_query} \n mapping query: {insert_query1} \n sample row: {rows[0] if rows else 'No rows to insert'} \n sample mapping row: {mapping_rows[0] if mapping_rows else 'No mapping rows to insert'}", flush=True)
        #print("Query:", insert_query)
        #print("Sample Row:", rows[0] if rows else "No rows to insert")
        print('Data and mapping table insert failed for medical_histories - New insert!')

# Insert new records and mapping entries simultaneously
insert_new_records_and_mapping(src_medical_histories_df1, tgt_connection, practice_name, table_name)
