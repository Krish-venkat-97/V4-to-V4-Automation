...
table_name = 'letter_categories'

src_letter_categories  = f"SELECT * FROM letter_categories"
src_letter_categories_df = pd.read_sql(src_letter_categories, src_connection)
#drop the duplicate names (case-insensitive)
src_letter_categories_df['name_upper'] = src_letter_categories_df['name'].str.upper()
src_letter_categories_df = src_letter_categories_df.drop_duplicates(subset=['name_upper'], keep='first')

tgt_letter_categories = 'SELECT id as target_id,UPPER(name) as letter_category_name FROM letter_categories'
tgt_letter_categories_df = pd.read_sql(tgt_letter_categories, tgt_connection)
...
src_letter_categories_df['name_upper'] = src_letter_categories_df['name'].str.upper()
src_letter_categories_df2 = pd.merge(src_letter_categories_df, tgt_letter_categories_df, left_on='name_upper', right_on='letter_category_name', how='inner')
src_letter_categories_df2 = src_letter_categories_df2.drop(columns=['name_upper', 'letter_category_name'])
src_letter_categories_df2 = src_letter_categories_df2[['target_id','id']].rename(columns={'id': 'source_id'})
...
def insert_new_records_and_mapping(df, tgt_connection, practice_name, table_name):
    ...
    try:
        ...
        print('Data and mapping table insert successful for letter_categories - New insert!')
    except Exception as e:
        logging.error(f"Data and mapping table insert failed for letter_categories - New insert! Error occurred: {e} \n Query: {insert_query} \n Mapping Query: {insert_query1} \n sample row: {rows[0] if rows else 'No rows to insert'} \n sample mapping row: {mapping_rows[0] if mapping_rows else 'No mapping rows to insert'}")
        print("Query:", insert_query)
        print("Sample Row:", rows[0] if rows else "No rows to insert")
        print('Data and mapping table insert failed for letter_categories - New insert!')

# Insert new records and mapping entries simultaneously
insert_new_records_and_mapping(src_letter_categories_df1, tgt_connection, practice_name, table_name)
...
def update_existing_records_and_mapping(df, tgt_connection, practice_name, table_name):
    ...
    try:
        with tgt_connection.cursor() as cursor:
            ...
            cursor.executemany(insert_query1, mapping_rows)
            tgt_connection.commit()
        print('Data and mapping table insert successful for letter_categories - Existing update!')
    except Exception as e:
        logging.error(f"Data and mapping table insert failed for letter_categories - Existing update! Error occurred: {e} \n Query: {insert_query1} \n sample row: {rows[0] if rows else 'No rows to insert'} \n sample mapping row: {mapping_rows[0] if mapping_rows else 'No mapping rows to insert'}")
        print("Query:", insert_query1)
        print("Sample Row:", rows[0] if rows else "No rows to insert")
        print('Data and mapping table insert failed for letter_categories - Existing update!')

# Update existing records and mapping entries simultaneously
update_existing_records_and_mapping(src_letter_categories_df2, tgt_connection, practice_name, table_name)
