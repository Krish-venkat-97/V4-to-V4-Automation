...
src_letters  = f"SELECT * FROM letters WHERE patient_id IN ({','.join(map(str, patient_ids))})"
src_letters_df = pd.read_sql(src_letters, src_connection)
...
tgt_mapping_letter_category_table = f"SELECT source_id, target_id FROM mapping_table WHERE source = '{getPractice()}' AND table_name = 'letter_categories'"
tgt_mapping_letter_category_table_df = pd.read_sql(tgt_mapping_letter_category_table, tgt_connection)
...
#---------------------------patient_id----------------------------------
# Merging source letters with target patients mapping to get target patient_id
src_letters_df['patient_id'] = src_letters_df['patient_id'].astype(int)
tgt_mapping_patient_table_df['source_id'] = tgt_mapping_patient_table_df['source_id'].astype(int)
src_letters_df = pd.merge(src_letters_df, tgt_mapping_patient_table_df, left_on='patient_id', right_on='source_id', how='left')
src_letters_df = src_letters_df.drop(columns=['patient_id', 'source_id'])
src_letters_df = src_letters_df.rename(columns={'target_id': 'patient_id'})

#----------------------------letter category---------------------------
# Merging source letters with target letter categories mapping to get target letter_category_id
if src_letters_df['letter_category_id'].isnull().all():
    src_letters_df['letter_category_id'] = 1
else:
    src_letters_df['letter_category_id'] = src_letters_df['letter_category_id'].apply(lambda x: int(x) if pd.notnull(x) else 1)
    tgt_mapping_letter_category_table_df['source_id'] = tgt_mapping_letter_category_table_df['source_id'].apply(lambda x: int(x) if pd.notnull(x) else 1)
    src_letters_df = pd.merge(src_letters_df, tgt_mapping_letter_category_table_df, left_on='letter_category_id', right_on='source_id', how='left')
    src_letters_df = src_letters_df.drop(columns=['letter_category_id', 'source_id'])
    src_letters_df = src_letters_df.rename(columns={'target_id': 'letter_category_id'})

#----------------------------episode_id-----------------------------------
# Merging source letters with target episodes mapping to get target episode_id
if src_letters_df['episode_id'].isnull().all():
    src_letters_df['episode_id'] = None
else:
    src_letters_df['episode_id'] = src_letters_df['episode_id'].apply(lambda x: int(x) if pd.notnull(x) else None)
    ...
    src_letters_df = pd.merge(src_letters_df, tgt_mapping_episode_table_df, left_on='episode_id', right_on='source_id', how='left')
    src_letters_df = src_letters_df.drop(columns=['episode_id', 'source_id'])
    src_letters_df = src_letters_df.rename(columns={'target_id': 'episode_id'})

#----------------------------wrtting default values--------------------
src_letters_df['template_id'] = None
src_letters_df['workflow_state_id'] = 0
src_letters_df['clinic_workflow_id'] = 0
src_letters_df['dictate_it_followup'] = 0
src_letters_df['from_dictate_it'] = 0
src_letters_df['verified_date'] = 0
src_letters_df['email_sent_date'] = 0
...
insert_new_records_and_mapping(src_letters_df1, tgt_connection, practice_name, table_name)
